...
try:
    from user_data.strategies.OnChainOracle import OnChainOracle
except ImportError:
    # Fallback for different path structures
    user_data_path = Path(__file__).resolve().parent.parent.parent
...
class NFI5MOHO_WIP(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative_1h_date = informative_1h['date']
        informative_1h = informative_1h.shift(1)
        informative_1h['date'] = informative_1h_date
        ...
        if OnChainOracle:
            dataframe = OnChainOracle().merge_with_dataframe(dataframe)
            
            # Calculate 3-day Rate of Change (Velocity) for Stablecoins (3 days * 24 hours * 12 5m-candles = 864)
            if 'total_tvl' in dataframe.columns and 'stable_mcap' in dataframe.columns:
                # Stablecoin Velocity (3-day ROC) - Captures "Dry Powder" momentum
                dataframe['stable_velocity'] = dataframe['stable_mcap'].pct_change(periods=864)
                
                # TVL Divergence (3-day ROC)
                dataframe['tvl_velocity'] = dataframe['total_tvl'].pct_change(periods=864)
                
                # Fill NaNs
                dataframe['stable_velocity'] = dataframe['stable_velocity'].fillna(0)
                dataframe['tvl_velocity'] = dataframe['tvl_velocity'].fillna(0)
                
                # Risk-Off Logic: Negative Stablecoin Velocity suggests capital outflows
                dataframe['risk_off'] = ((dataframe['stable_velocity'] < 0) | (dataframe['tvl_velocity'] < -0.01)).astype(int)
            else:
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Conditions 20 and 21 disabled - redundant with other RSI dip conditions
        # conditions.append(
        #     (
        ...
        if self.enable_onchain_filter.value:
            if 'risk_off' in dataframe.columns:
                dataframe.loc[dataframe['risk_off'] == 1, 'buy'] = 0
